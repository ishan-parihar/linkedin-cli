...
class ObscuraMetricsCollector:
    ...
    def record_operation(
        ...
        logger.debug("Recorded operation metric: %s (%.3fs, success=%s)", operation, duration, success)
    ...
    def get_session_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self._session_metrics,
            ...
            "operations_per_second": self._session_metrics["total_operations"] / uptime if uptime > 0 else 0,
            "overall_success_rate": (
                self._session_metrics["successful_operations"] / self._session_metrics["total_operations"]
                if self._session_metrics["total_operations"] > 0 else 0
            ),
...
class ObscuraStructuredLogger:
    ...
    def __init__(self, log_file: Optional[Path] = None):
        self.log_file = log_file or Path.home() / ".linkedin-lyr" / "obscura_logs" / f"obscura_{datetime.now().strftime('%Y%m%d')}.jsonl"
