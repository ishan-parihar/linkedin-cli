...
def handle_obscura_error(error: Exception, context: str) -> None:
    ...
    if isinstance(error, subprocess.TimeoutExpired):
        logger.error("Obscura timeout in %s: %s. Consider increasing timeout.", context, str(error))
    elif isinstance(error, subprocess.CalledProcessError):
        logger.error(
            "Obscura process error in %s: returncode=%d, stderr=%s",
            context,
            error.returncode,
            error.stderr[:200] if error.stderr else "empty",
        )
    else:
...
def log_obscura_performance(
    operation: str, duration_seconds: float, details: dict[str, Any] | None = None
) -> None:
    """Log Obscura performance metrics."""
    perf_data = {"operation": operation, "duration_seconds": duration_seconds, **(details or {})}
...
def log_fallback_attempt(original_backend: str, fallback_backend: str, reason: str) -> None:
    ...
    logger.warning(
        "Backend fallback: %s -> %s (reason: %s)", original_backend, fallback_backend, reason
    )
