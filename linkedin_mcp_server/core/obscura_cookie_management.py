...
class ObscuraCookieManager:
    ...
    def __init__(self, cookie_file: Optional[Path] = None):
        self.cookie_file = cookie_file or Path.home() / ".linkedin-mcp" / "cookies.json"
    ...
    async def validate_session(self) -> bool:
        ...
        if self._last_validation and (time.time() - self._last_validation) < self._validation_interval:
            return True
...
class ObscuraSessionManager:
    ...
    def __init__(self, session_dir: Optional[Path] = None):
        self.session_dir = session_dir or Path.home() / ".linkedin-mcp" / "sessions"
    ...
    def get_session_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "total_requests": sum(s.get("request_count", 0) for s in self._active_sessions.values()),
...
class CookieOptimizer:
    ...
    def get_optimal_cookie_set(self, all_cookies: dict[str, str]) -> dict[str, str]:
        ...
        for cookie_name, usage_count in sorted(
            self._cookie_usage_stats.items(),
            key=lambda x: x[1],
            reverse=True
        ):
        ...
        logger.info(
            "Optimized cookie set: %d cookies from %d total",
            len(optimal_cookies), len(all_cookies)
        )
