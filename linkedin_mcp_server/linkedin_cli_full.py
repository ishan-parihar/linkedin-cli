...
Run with commands like: linkedin-lyr status, linkedin-lyr import, etc.
...
def cmd_status(full: bool = False) -> None:
    ...
    if not cookie_path.exists():
        toon_output({
            ...
            "session": "none",
            "message": "No LinkedIn session found. Run 'linkedin-lyr import' to set up authentication.",
        })
        toon_help([
            "Run 'linkedin-lyr import' to import cookies from your browser",
            "Run 'linkedin-lyr browsers' to list supported browsers",
        ])
        return
    ...
    try:
        ...
        if not li_at_found:
            toon_help([
                "Run 'linkedin-lyr import' to refresh your session",
                "Run 'linkedin-lyr logout' to clear the invalid session",
            ])
...
def cmd_browsers() -> None:
    ...
    toon_help([
        "Run 'linkedin-lyr import' to auto-detect and import from available browsers",
...
def cmd_import(browser: str = None) -> None:
    ...
    if not cookie_data:
        toon_error(
            "no LinkedIn cookies found in any browser", "log into LinkedIn in your browser first"
        )
        sys.exit(1)
    ...
    toon_help([
        "Run 'linkedin-lyr status' to verify your session",
...
def cmd_logout() -> None:
    ...
    try:
        ...
        toon_error(f"failed to clear session: {e}", "check file permissions")
        sys.exit(1)
